...
async function upgradeAppImpl({ web3, artifacts, appName = APP }) {
  ...
  const appAddress = state[`app:${APP}`].proxyAddress

  const nodeOperatorRegistry = await artifacts.require('NodeOperatorsRegistry').at(appAddress)
  ...
  log(`Using nodeOperatorRegistry proxy address:`, yl(appAddress))
  ...
  const callData1 = encodeCallScript([
    ...
    {
      // kernel.setApp(APP_BASES_NAMESPACE, appId, nodeOperatorRegistry)
      to: state.daoAddress,
